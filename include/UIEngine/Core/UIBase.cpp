...
HWND CWindowWnd::Create(HWND hwndParent, LPCTSTR pstrName, DWORD dwStyle, DWORD dwExStyle, int x, int y, int cx, int cy, HMENU hMenu)
{
    ...
    LPCTSTR sClassName = GetWindowClassName();
    m_hWnd = ::CreateWindowEx(dwExStyle,
        sClassName/*GetWindowClassName()*/,
        pstrName,
        dwStyle,
        x,
        y,
...
UINT CWindowWnd::ShowModal()
{
    ...
    while( ::IsWindow(m_hWnd) && ::GetMessage(&msg, NULL, 0, 0) ) 
    {
        ...
        {
            // [2016-4-13 lxj]
            // 将虚拟键消息转换为字符消息，字符消息被寄送到调用线程的消息队列中
            // 当下一次线程调用函数GetMessage或PeekMessage时字符消息被读出
...
LRESULT CALLBACK CWindowWnd::__WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    // [2016-4-13 lxj]
    // 创建窗口时会发出WM_CREATE消息，但在发出这个消息之前，会发出WM_NCCREATE消息 
    // 对应的，在关闭窗口时会发出WM_DESTROY,WM_NCDESTROY消息，顺序为WM_DESTORY,WM_NCDESTORY
    if( uMsg == WM_NCCREATE ) 
    {
